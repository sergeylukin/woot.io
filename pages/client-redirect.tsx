...
const Page = () => {
  ...
  return (
    <>
      ...
      between pages by using the Provider in _app.js so that navigation between
      pages using useSession() is very fast. The disadvantage of useSession() is
      that it requires client side JavaScript.
      <blockquote>
        <p>This page is protected using the useSession hook.</p>
        <p>Either way works.</p>
        <p>
          But in this case the session is <strong>not</strong> available on the
          first render.
        </p>
      </blockquote>
    </>
